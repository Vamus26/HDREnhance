...
public:
	T* pData;
protected:
	mwSize imWidth,imHeight,nChannels;
	mwSize nPixels,nElements;
	bool IsDerivativeImage;
	color_type colorType;
public:
	Image(void);
	Image(mwSize width, mwSize height, mwSize nchannels=1);
	Image(const T& value, mwSize _width, mwSize _height, mwSize _nchannels=1);
	...
	virtual void allocate(mwSize width, mwSize height, mwSize nchannels=1);
	...
	void setValue(const T& value, mwSize _width, mwSize _height, mwSize _nchannels=1);
	T immax() const
	{
		T Max=pData[0];
	...
	inline const T& operator [] (mwSize index) const {return pData[index];};
	inline T& operator[](mwSize index) {return pData[index];};

	inline T*& data(){return pData;};
	inline const T*& data() const{return (const T*&)pData;};
	inline mwSize width() const {return imWidth;};
	inline mwSize height() const {return imHeight;};
	inline mwSize nchannels() const {return nChannels;};
	inline mwSize npixels() const {return nPixels;};
	inline mwSize nelements() const {return nElements;};
	...
	bool matchDimension (mwSize width, mwSize height, mwSize nchannels) const;
	...
	void imresize(mwSize dstWidth, mwSize dstHeight);
	template <class T1>
	void imresize(Image<T1>& result, mwSize dstWidth, mwSize dstHeight) const;
...
//------------------------------------------------------------------------------------------
template <class T>
Image<T>::Image(mwSize width, mwSize height, mwSize nchannels)
{
	imWidth=width;
	imHeight=height;
	nChannels=nchannels;
...
template <class T>
Image<T>::Image(const T& value, mwSize _width, mwSize _height, mwSize _nchannels)
{
	pData=NULL;
...
template <class T>
void Image<T>::allocate(mwSize width, mwSize height, mwSize nchannels)
{
	clear();
	imWidth=width;
	imHeight=height;
...
template <class T>
void Image<T>::setValue(const T& value, mwSize _width, mwSize _height, mwSize _nchannels)
{
...
template <class T>
bool Image<T>::matchDimension(mwSize width, mwSize height, mwSize nchannels) const
{
...
template <class T>
template <class T1>
void Image<T>::imresize(Image<T1>& result, mwSize DstWidth, mwSize DstHeight) const
{
	if(result.width()!=DstWidth || result.height()!=DstHeight || result.nchannels()!=nChannels)
		result.allocate(DstWidth,DstHeight,nChannels);
	else
...
template <class T>
void Image<T>::imresize(mwSize dstWidth, mwSize dstHeight)
{
...
{
	mwSize width = imWidth*ratio;
	mwSize height = imHeight*ratio;
	if(!output.matchDimension(width,height,nChannels))
		output.allocate(width,height,nChannels);
	for(int i =  0; i <imHeight; i++)
		for(int j = 0; j<imWidth; j++)
		{
			mwSize offset = (i*imWidth+j)*nChannels;
			for(int ii = 0 ;ii<ratio;ii++)
				for(int jj=0;jj<ratio;jj++)
				{
					mwSize offset1 = ((i*ratio+ii)*width+j*ratio+jj)*nChannels;
					for(int k = 0; k<nChannels; k++)
