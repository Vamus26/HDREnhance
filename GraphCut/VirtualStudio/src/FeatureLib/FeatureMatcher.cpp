...
{
	unordered_set<int> queryFeaTypes;
	unordered_set<int> searchFeaTypes;
	...
	for(unordered_set<int>::iterator queryType = queryFeaTypes.begin();
...
void FeatureMatcher::exhaustiveSearch(FeatureMatches &matches, FeatureMatcherParams params, Feature::FEATURE_TYPE featureType)
{
	...
	unordered_map<uint, InverseNeighbour> inverseNeighbours;
	unordered_map<uint, InverseNeighbour>::iterator findResult;
	...
	for(unordered_map<uint, InverseNeighbour>::const_iterator iMatch = inverseNeighbours.begin(); 
