...
% load the frames
for cnt=1:100
    tmp = sprintf('%d.exr',cnt);
    tmpL = sprintf('%d.jpg',cnt);
    if ~isfile(tmpL)
        amount = cnt-1;
        break;
    end
...
for count=1:amount
    ...
    if imag1~=0
     ...
     elseif ~exist('secondHdr','var')
        secondHdr = imag1;
	 secondHdrIdx = count;
        doFlowCalculation(firstHdr,secondHdr,LdrStack, firstHdrIdx, secondHdrIdx);
       firstHdr =secondHdr;
       clear secondHdr;
       clear LdrStack;
     end
...
function [] =doFlowCalculation(firstHdr,secondHdr,LdrStack, firstHdrIdx, secondHdrIdx)
...
function [FWDImage, BWDImage, moco_fwd, moco_bwd] =createHDR(leftHdr, rightHdr,vx_1,vy_1,vxb_1,vyb_1)
	[x, y] = meshgrid(1:size(leftHdr), 1:size(leftHdr));
...
function [] =doGraphCut(FWDImage, BWDImage, moco_bwd, moco_fwd, firstHdrIdx, secondHdrIdx, LdrFrame, ldrIdx)
...
rgb = tonemap(mix);
hdrfname = sprintf ( 'CreatedHDR_%d.jpg', ldrIdx );
ldrfname = sprintf ( 'CreatedHDR_%d.jpg', ldrIdx );
