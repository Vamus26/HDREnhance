...
//	
//--------------------------------------------------------------------------------------------------------
void OpticalFlow::SmoothFlowSORPlusThird(const DImage &Im1, const DImage &Im2, DImage &warpIm2, DImage &u, DImage &v,
	double alpha, mwSize nOuterFPIterations, mwSize nInnerFPIterations, mwSize nSORIterations, const DImage &Im3, DImage &warpIm3)
{
	DImage mask, imdx, imdy, imdt;
	mwSize imWidth, imHeight, nChannels, nPixels;
	imWidth = Im1.width();
	imHeight = Im1.height();
...
void OpticalFlow::SmoothFlowSOR(const DImage &Im1, const DImage &Im2, DImage &warpIm2, DImage &u, DImage &v, 
																    double alpha, mwSize nOuterFPIterations, mwSize nInnerFPIterations, mwSize nSORIterations)
{
	DImage mask,imdx,imdy,imdt;
	mwSize imWidth,imHeight,nChannels,nPixels;
	imWidth=Im1.width();
...
void OpticalFlow::SmoothFlowPDE(const DImage &Im1, const DImage &Im2, DImage &warpIm2, DImage &u, DImage &v, 
																    double alpha, mwSize nOuterFPIterations, mwSize nInnerFPIterations, mwSize nCGIterations)
{
	DImage mask,imdx,imdy,imdt;
	mwSize imWidth,imHeight,nChannels,nPixels;
	imWidth=Im1.width();
...
//--------------------------------------------------------------------------------------
// ORIGINAL function to perfomr coarse to fine optical flow estimation
//--------------------------------------------------------------------------------------
void OpticalFlow::Coarse2FineFlowOrig(DImage &vx, DImage &vy, DImage &warpI2, const DImage &Im1, const DImage &Im2, double alpha, double ratio, int minWidth,
	...
	for(int k=GPyramid1.nlevels()-1;k>=0;k--)
	{
		...
		else
		{
			...
			if (interpolation == Bilinear) {
				...
				mexErrMsgTxt("removed To work optical flow.cpp:996");
...
//--------------------------------------------------------------------------------------
// OWN function to perfomr coarse to fine optical flow estimation
//--------------------------------------------------------------------------------------
void OpticalFlow::Coarse2FineFlowExt(DImage &vx, DImage &vy, DImage &warpI2, const DImage &Im1, const DImage &Im2, double alpha, double ratio, int minWidth,
	int nOuterFPIterations, int nInnerFPIterations, int nCGIterations, DImage &warpI3, const DImage& Im3, const DImage& Im4)
{
	// first build the pyramid of the two images
	GaussianPyramid GPyramid1;
...
void OpticalFlow::Coarse2FineFlowLevel(DImage &vx, DImage &vy, DImage &warpI2,const DImage &Im1, const DImage &Im2, double alpha, double ratio, mwSize nLevels,
